...
class SecureStorage:
    ...
    def report_bug(self) -> int:
        ...
        if self.debug:
            ...
            print("args:", self.store_data_args)
            ...
            if len(self.store_data_args) >= 10 and len(self.data) == 1:
                ...
                print(min(key_lengths), max(key_lengths), len(self.store_data_args))
...
for i in range(22, 32):
    inp.append(json.dumps({"action": "import_key", "key_id": f"key{i}", "key": '00' * i}))
    inp.append(json.dumps({"action": "store_data", "key_id": f"key{i}", "data": ""}))
inp.append(json.dumps({"action": "report_bug"}))
