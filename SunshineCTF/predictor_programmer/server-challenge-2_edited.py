...
flag_p2 = b"sun{fake flag}"#open('./key', "rb").read()

debug_file = open("server-challenge-2_stderr.txt", "w+")

def test_client(stdinput):
    seed = get_time_number()
    print(seed, file=debug_file, flush=True)
    ...
    while lives > 0 and correct_responses < 5:
        ...
        print(next_number, file=debug_file, flush=True)
