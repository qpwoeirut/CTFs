# mostly copied from https://pyokagan.name/blog/2019-10-14-png/
...
Recon = np.array(Recon, dtype=np.uint8).reshape((height, width, 4))
Image.fromarray(Recon).save("out.png")


print([(chunk[0], len(chunk[1])) for chunk in chunks])
print(len(IDAT_data), height * width * 4)  # increase height until these two numbers are close together
assert len(chunks) == 19
# 19 chunks counted from https://www.nayuki.io/page/png-file-chunk-inspector
