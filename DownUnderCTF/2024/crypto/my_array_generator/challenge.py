...
class MyArrayGenerator:
    ...
    def encrypt(self, plaintext: bytes) -> bytes:
        ...
        for i, b in enumerate(plaintext):
            self.update()
            assert self.carry == self.registers[-2] ^ self.registers[-1]
            assert self.registers[0] ^ self.registers[-3] ^ self.registers[-1] in (0, 0xffff_ffff)
            if i % 127 <= 5:
                print(i, [hex(x)[2:].zfill(8) for x in self.registers[:4]], [hex(x)[2:].zfill(8) for x in self.registers[-5:]])
                print(hex(self.registers[0] ^ self.carry ^ self.registers[-3] ^ self.registers[-2])[2:].zfill(8), hex(self.carry)[2:].zfill(8))
