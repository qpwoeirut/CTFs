...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
        ...
        if (actionMasked != 0) {
            if (actionMasked == 1) {
                ...
                if (findPointerIndex == -1) {
                    ...
                    int releaseVerticalGlow = i2 - releaseVerticalGlow(i2, motionEvent2.getX(findPointerIndex));
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        int i5 = scrollRange;
                        boolean z2 = overScrollByCompat(0, i4, 0, getScrollY(), 0, scrollRange, 0, 0, true) && !hasNestedScrollingParent(0);
                        int scrollY2 = getScrollY() - scrollY;
                        ...
                        if (z) {
                            int i8 = i4 - this.mScrollConsumed[1];
                            int i9 = scrollY + i8;
                            if (i9 < 0) {
                                EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, ((float) (-i8)) / ((float) getHeight()), motionEvent2.getX(findPointerIndex) / ((float) getWidth()));
                                if (!this.mEdgeGlowBottom.isFinished()) {
                                    this.mEdgeGlowBottom.onRelease();
                                }
                            } else if (i9 > i5) {
                                EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, ((float) i8) / ((float) getHeight()), 1.0f - (motionEvent2.getX(findPointerIndex) / ((float) getWidth())));
    ...
    public int consumeFlingInVerticalStretch(int i) {
        ...
        if (i > 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowTop) != 0.0f) {
            int round = Math.round((((float) (-height)) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (((float) (-i)) * FLING_DESTRETCH_FACTOR) / ((float) height), MAX_SCROLL_FACTOR));
            if (round != i) {
            ...
            int round2 = Math.round((f / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, (((float) i) * FLING_DESTRETCH_FACTOR) / f, MAX_SCROLL_FACTOR));
            if (round2 != i) {
