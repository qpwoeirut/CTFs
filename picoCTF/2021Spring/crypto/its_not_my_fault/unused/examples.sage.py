...
# This file was *autogenerated* from the file unused/examples.sage
from sage.all_cmdline import *   # import sage library

_sage_const_p3 = RealNumber('.3'); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_7 = Integer(7); _sage_const_p15 = RealNumber('.15'); _sage_const_p12 = RealNumber('.12'); _sage_const_p25 = RealNumber('.25'); _sage_const_1024 = Integer(1024); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_p05 = RealNumber('.05'); _sage_const_128 = Integer(128); _sage_const_32 = Integer(32)# load('coppersmith.sage')

def univariate(N):
	bounds = (floor(N**_sage_const_p3 ),)
	roots = tuple(randrange(bound) for bound in bounds)
	R = Integers(N)
	P = PolynomialRing(R, _sage_const_1 , names=('x',)); (x,) = P._first_ngens(1)
	monomials = [x, x**_sage_const_2 , x**_sage_const_3 ]
	f = sum(randrange(N)*monomial for monomial in monomials)
	f -= f(*roots)
	return small_roots(f, bounds, m=_sage_const_7 )
...
def boneh_durfee(N, p, q):
	...
	return small_roots(f, bounds, m=_sage_const_3 , d=_sage_const_4 )
...
def approximate_factor(N, p):
	...
	f += p - f(*roots)
	return small_roots(f, bounds, m=_sage_const_2 , d=_sage_const_4 )
